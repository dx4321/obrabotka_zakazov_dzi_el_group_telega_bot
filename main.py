"""
Необходимо написать код проекта сервиса обработки заказов для телеграм бота.

Используемые технологии python 3.8
peewee должен использовать sql lite, просто так как отдельню библиотеку sql lite использовать не надо
aiogram


Должно быть так

При входе в боте /start
    бот запрашивает Ваш контактный номер телефона
    затем бот запрашивает Ваше ФИО

После авторизации бот предоставляет 3 ReplyKeyboardMarkup основные кнопки
    1 Мои заказы
    2 Обращения
    3 Техническая поддержка

После перехода по кнопке 1, должны появиться 2 инлайн кнопки а именно Создать заказ / просмотреть мои заказы
После перехода по кнопке 2, должны появиться 2 инлайн кнопки а именно Создать обращени / просмотреть обращения
После нажатия на 3 кнопку, должно прийти пользователю сообщения о том что начат поиск опиратора
...
•	Описание: Клиенты могут отправлять заявки на покупку сухих бульонов и ингредиентов через телеграмм-бот. Заявка содержит информацию о выбранных продуктах и их количестве.
Оперативные документы:
•	Статус заказов:
•	Формат: Экранные формы в телеграмм-боте.
